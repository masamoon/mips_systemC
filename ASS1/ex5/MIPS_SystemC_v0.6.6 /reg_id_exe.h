...
SC_MODULE(reg_id_exe_t) {
	...
	regT < sc_uint<5> > *rs_id2;
	regT < sc_uint<5> > *rt_id2;
	
//	regT < sc_uint<5> > *rs_exe; 
//	regT < sc_uint<5> > *rt_exe; 
	...
	SC_CTOR(reg_id_exe_t) {
		...
		rs_id2 = new regT< sc_uint<5> >("rs_id2");
		rs_id2->din(rs_id); 
		rs_id2->dout(rs_exe);
		rs_id2->clk(clk);
		rs_id2->enable(enable);
		rs_id2->reset(reset);

		rt_id2 = new regT< sc_uint<5> >("rt_id2");
		rt_id2->din(rt_id);
		rt_id2->dout(rt_exe);
		rt_id2->clk(clk);
		rt_id2->enable(enable);
		rt_id2->reset(reset);
